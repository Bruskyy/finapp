...
- **SQL Server (Lancamentos)** — este foi o ponto sem solução gratuita "de verdade": não existe hoje hospedagem gerenciada de SQL Server sem exigir cartão de crédito no cadastro (nem rodar o `mssql-server-linux` num container do Render funcionaria — o free tier não tem disco persistente, o banco seria apagado a cada hibernação). Optado por **Azure SQL Database** (tier serverless, free offer) — cartão cadastrado na conta Azure, mas sem sair do plano gratuito.
  ...
  **Incidente real (15/07/2026): o keep-alive estourou a cota gratuita mensal do Azure SQL em ~15 dias, sem tráfego de usuário nenhum.** O tier serverless do Azure SQL tem **auto-pause** (economiza a cota de computação quando ocioso) — mas os 4 serviços com banco tinham `AddDbContextCheck` no `/health`, e o `keep-alive.yml` pinga `/health` a cada 10 minutos. Isso resetava o auto-pause continuamente, mantendo o banco "ativo" o tempo inteiro e consumindo a cota de computação sem parar. O Azure pausou o banco de vez (`Error Number:42119`) até renovar em 1º do mês seguinte — `Lancamentos.Api` ficou fora do ar (toda tentativa de `Database.Migrate()` no boot lançava exceção não tratada e derrubava o processo). **Fix:** `AddDbContextCheck` removido dos 4 serviços — `/health` virou liveness pura (processo de pé), sem tocar em banco nenhum; o keep-alive continua mantendo os *Web Services* do Render quentes, só parou de acordar os bancos à força. O mesmo padrão (Postgres/Neon) provavelmente estava consumindo cota de forma equivalente nos outros 3 serviços, só sem ainda ter estourado — corrigido preventivamente nos 4 de uma vez.
